...
{
    private static readonly Lazy<Dictionary<LoanExtensionStatus, string>> LoanExtensionStatusDescriptions = new(() => new Dictionary<LoanExtensionStatus, string>
    {
    ...
    public static string GetDescription(Enum value)
    {
        ...
        // Use type checking to select the correct dictionary
        if (value is LoanExtensionStatus loanExtensionStatus)
        {
            if (LoanExtensionStatusDescriptions.Value.TryGetValue(loanExtensionStatus, out var desc))
                return desc;
        }
        else if (value is LoanReturnStatus loanReturnStatus)
        {
            if (LoanReturnStatusDescriptions.TryGetValue(loanReturnStatus, out var desc))
                return desc;
        }
        else if (value is MembershipRenewalStatus membershipRenewalStatus)
        {
            if (MembershipRenewalStatusDescriptions.TryGetValue(membershipRenewalStatus, out var desc))
                return desc;
        }
